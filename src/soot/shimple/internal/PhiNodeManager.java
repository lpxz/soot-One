...
public class PhiNodeManager
{
     ...
     **/
    public static HashSet<AssignStmt> lp_phiDecodedDefStmts_pl = new HashSet<AssignStmt>(); 
    public boolean doEliminatePhiNodes()
    {
    	lp_phiDecodedDefStmts_pl.clear(); // this is a hack by peng liu. just want to reuse them to decode phi in constraint encoding. Query after this method.
        ...
        for(int i = 0; i < equivStmts.size(); i++){
            ...
            if(pred.branches()){
                ...
                lp_phiDecodedDefStmts_pl.add(stmt);
            }
            else
            {
