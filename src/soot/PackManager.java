...
public class PackManager {
    ...
    public void writeOutputExcept( List<String> excludelist ) {
        ...
        Iterator<SootClass> it= reachableClasses() ;
           Set<SootClass> remains = new HashSet<SootClass>();
           while (it.hasNext()) {
			SootClass sootClass = (SootClass) it.next();
			boolean shouldExclude = false;
			for(String exclude:excludelist )
			{
				if(sootClass.getName().startsWith(exclude))
				{
					shouldExclude= true;
				}
			}
            ...
            if( Options.v().output_format() == Options.output_format_dava ) {
                ...
                writeOutput(remains.iterator());
            }
            postProcessXML(remains.iterator());
            releaseBodies( remains.iterator() );
        if(Options.v().verbose())
            PhaseDumper.v().dumpAfter("output");
